...
class AccountScrubber:
    """Handles the entire account scrubbing workflow from a clean slate."""
    ...
    def _score_candidate(self, scrub_row, db_row):
        ...
        REVISED: Replaces knockout rules with a configurable penalty system.
        """
        score, details = 0, []
    ...
    def run(self):
        ...
        # 1. LOAD DATA
        print("\n[Stage 1/4] Loading and standardizing data...")
        ...
        accounts_map = {
            ...
            'owner.name': 'owner_name', 'ownerid': 'owner_id',
            'account_status__c': 'account_status', 'total_open_opps__c': 'total_open_opps'
        }
...
class ContactScrubber:
    """
    Handles the entire contact scrubbing workflow. This is a standalone class.
    """

    def __init__(self, config, filename):
        # This scrubber operates on the OUTPUT of the account scrub
        self.config = config
        self.paths = config['Paths']
        ...
        # The input is an account scrub output file, located in the output directory
    ...
    def run(self):
        """Executes highly efficient fuzzy matching workflow for contacts."""
        total_start_time = time.time()
        print("\n" + "="*50); print("          CONTACT SCRUBBING WORKFLOW START"); print("="*50)

        # 1. LOAD DATA
        print("\n*** STAGE 1: Loading Input Data and Target Accounts ***")
        # The input is the result of the account scrub. `data_io` handles lowercasing.
        scrub_df = data_io.load_and_standardize_excel(self.input_path)
        scrub_df['original_index'] = scrub_df.index # Use index to guarantee a unique key

        # The contact list is the master list of all contacts.
        contacts_db = data_io.load_and_standardize_excel(self.contact_list_path)

        # Find which accounts we need to search for contacts in
        # The column name 'matched_accountid' is created by your working AccountScrubber
        ...
        # Filter the main contact DB down to only contacts from the matched accounts
        ...
        # Group candidates by account ID for extremely fast lookups
        ...
        all_best_matches = []
        # We only need to search rows that have a matched account
        ...
        for _, scrub_row in records_to_match.iterrows():
            ...
            for candidate_row in candidates:
                ...
                if score > highest_score:
                    highest_score = score
                    # Store the details of the best match found so far
                    best_candidate_details = {
                        'original_index': scrub_row['original_index'], # This is the key for merging back
        ...
        if all_best_matches:
            ...
            # Use the foolproof original_index to merge results back
